...
@dag(
    dag_id="raw_api1_collect_backfill",
    schedule="@daily",
...
def extract_and_load_kamis_api1():  # noqa: D103
    @task(max_active_tis_per_dag=10)
    def fetch_and_save(req: APIRequestParams, data_interval_start=None) -> Optional[str]:  # noqa: ANN001
        """
        API 호출과 저장을 하나의 원자적(Atomic) 작업으로 통합.
        ...
        if not data_block:
            logger.error(f"No data block in response: {json_data}")
        ...
        if isinstance(data_block, dict):
            ...
            if error_code == "000":
                ...
                logger.error(f"KAMIS API Error - Code: {error_code}, Message: {error_msg}")
                # 재시도 불필요 - API가 명시적으로 에러 응답
                raise AirflowSkipException(f"KAMIS API Error: {error_code} - {error_msg}")

        elif isinstance(data_block, list):
            if len(data_block) == 0:
                logger.error("Empty data list returned from API - this is unexpected")
                # 재시도 불필요 - 응답 구조가 비정상
                raise AirflowSkipException("API returned empty data list - invalid response format")

            # 001은 데이터 없음(정상) - 저장은 하되 로그 남김
            if data_block[0] == "001":
                logger.info(f"No data available for {regday} (code 001) - Saving empty response")
                # 아래로 진행하여 S3에 저장
            else:
                logger.error(f"Unexpected API response code: {data_block[0]}")
                # 재시도 불필요 - 알 수 없는 응답 코드
                raise AirflowSkipException(f"KAMIS API returned unexpected code: {data_block[0]}")

        # 4. S3(MinIO) 저장 경로 생성 - 응답 메타데이터 기반
        try:
    ...
    fetch_and_save.expand(req=requests_list)
